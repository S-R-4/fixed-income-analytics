#This variation poses security risk if user enters malicious input
#B/C of the 'eval()' function
#Can input arithmetic equations '0.2+0.5'

def calculate_n_year_spot_rate():
    N = int(eval(input("Enter the number of years (N): ")))  # Allow arithmetic input
    ...
    if first_rate.lower() == 'na':
        spot_rates = []
        for i in range(N):
            rate = eval(input(f"Enter {i+1}YR Spot Rate 0 YEAR(s) from now (as a decimal, e.g., 0.05 for 5%): "))
            spot_rates.append(rate)
    else:
        forward_rates.append(eval(first_rate))  # Allow arithmetic input
        for i in range(1, N):
            rate = eval(input(f"Enter 1YR Forward Rate {i} YEAR(s) from now (as a decimal, e.g., 0.05 for 5%): "))
